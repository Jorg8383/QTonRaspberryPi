...
RUN { \
    ...
    cd qt6/src && \
    wget https://download.qt.io/official_releases/qt/6.9/6.9.3/submodules/qtbase-everywhere-src-6.9.3.tar.xz && \
    wget https://download.qt.io/official_releases/qt/6.9/6.9.3/submodules/qtshadertools-everywhere-src-6.9.3.tar.xz && \
    wget https://download.qt.io/official_releases/qt/6.9/6.9.3/submodules/qtdeclarative-everywhere-src-6.9.3.tar.xz && \
    cd ../host-build && \
    tar xf ../src/qtbase-everywhere-src-6.9.3.tar.xz && \
    tar xf ../src/qtshadertools-everywhere-src-6.9.3.tar.xz && \
    tar xf ../src/qtdeclarative-everywhere-src-6.9.3.tar.xz && \
    echo "Compile qtbase for host" && \
    cd qtbase-everywhere-src-6.9.3 && \
    ...
    cd ../qtshadertools-everywhere-src-6.9.3 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    echo "Compile declerative for host" && \
    cd ../qtdeclarative-everywhere-src-6.9.3 && \
    /build/qt6/host/bin/qt-configure-module . && \
    cmake --build . --parallel 4 && \
    cmake --install . && \
    cd ../../pi-build && \
